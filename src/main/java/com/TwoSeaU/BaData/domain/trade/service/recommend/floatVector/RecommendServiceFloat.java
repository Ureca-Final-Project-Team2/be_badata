...
public class RecommendServiceFloat {
    ...
    public PostsResponse recommendPostsByFloat(String username, final boolean isStart) {
        if (username == null) {
    ...
    private PostsResponse recommendContentBasedFilteringByPgvector (User user, final boolean isStart) {
        ...
        List<RecommendationResult> results = candidatePosts.stream()
                .map(post -> {
                    float[] postVector = post.getFloatVector();

                    //유사도 계산
                    float similarity = vectorUtilsFloat.calculateWeightedSimilarity(userVector, postVector);

                    //최종 점수 계산 (추후 최종 점수에 인기도 반영)
                    float finalScore = similarity;
