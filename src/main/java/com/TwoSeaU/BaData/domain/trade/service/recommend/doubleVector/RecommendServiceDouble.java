...
public class RecommendServiceDouble {
    final static String REDIS_KEY = "rec:user:";
    final static int RECOMMEND_LIMIT = 10;
    final static int TTL_LIMIT = 30;

    private final UserProfileVectorizerDouble userProfileVectorizerDouble;
    ...
    private PostsResponse recommendContentBasedFiltering (User user, final boolean isStart) {
        ...
        double[] userVector = userProfileVectorizerDouble.vectorizeUserProfile(user, recommendedPostsId);
        ...
        List<RecommendationResult> results = candidatePosts.stream()
                .map(post -> {
                    double[] postVector = post.getDoubleVector();
